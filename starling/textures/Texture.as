	...
	public class Texture
	{
												 ...
												 scale:Number=1, format:String="bgra"):Texture
		{
			...
			if (asset is Bitmap)
			{
				...
				texture = Texture.fromBitmap(bitmap, mipMapping, false, scale, format);
				texture.root.onRestore = function():void
				{
			...
			{
				var assetBytes:ByteArray = asset as ByteArray;
				texture = Texture.fromAtfData(assetBytes, scale, mipMapping);
				texture.root.onRestore = function():void
				{
					texture.root.uploadAtfData(assetBytes); 
				};
				assetBytes.clear();
				assetBytes = null;
			}
...
//                 trace("useRectTexture: ",useRectTexture,"mipMapping",mipMapping);
			if (useRectTexture)
			{
			...
			if (isPot || useRectTexture)
				...
				return new SubTexture(concreteTexture, new Rectangle(0, 0, width, height),1.0, true);
		}
		
		/** Creates a texture that contains a region (in pixels) of another texture. The new
		 *  texture will reference the base texture; no data is duplicated. */
		public static function fromTexture(texture:Texture, region:Rectangle=null,
										   frame:Rectangle=null,XMLScale:Number = 1.0):Texture
		{
