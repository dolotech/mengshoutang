...
//            mVertexData = new VertexData(0, true);
            mVertexData =  VertexData.getInstance(0, true);
            mIndexData = new <uint>[];
            mNumQuads = 0;
            mTinted = false;
            mSyncRequired = false;
        ...
        public override function dispose():void {
            ...
            mIndexData.length = 0;
            mNumQuads = 0;
            VertexData.recycleInstance(mVertexData);
         ...
         *  the batch. */
        public function addQuad(quad:Quad, parentAlpha:Number = 1.0, texture:Texture = null, smoothing:String = null, modelViewMatrix:Matrix = null,
        ...
        public function isStateChange(tinted:Boolean, parentAlpha:Number, texture:Texture, smoothing:String, blendMode:String,
                                      numQuads:int = 1):Boolean {
            if (mNumQuads == 0)
                return false;
        ...
        private static var QuadBatchPool:Array = [];
        public static function recycleInstance(quadBatch:QuadBatch):void
        {
            if(QuadBatchPool.indexOf(quadBatch) == -1)
            {
                quadBatch.reset();
                QuadBatchPool.push(quadBatch);
            }
        }
        public static function getInstance():QuadBatch
        {
            var quadBatch:QuadBatch;
            if(QuadBatchPool.length > 0)
            {
                quadBatch  =  QuadBatchPool.pop();
            }
            else
            {
        ...
        private static function compileObject(object:DisplayObject, quadBatches:Vector.<QuadBatch>, quadBatchID:int, transformationMatrix:Matrix,
            ...
            if (filter && !ignoreCurrentFilter) {
                if (filter.mode == FragmentFilterMode.ABOVE) {
                    quadBatchID = compileObject(object, quadBatches, quadBatchID, transformationMatrix, alpha, blendMode,
                                                true);
                }

                quadBatchID = compileObject(filter.compile(object), quadBatches, quadBatchID, transformationMatrix, alpha,
                                            blendMode);
        ...
        private function getProgram(tinted:Boolean):Program3D {
            ...
            if (!program) {
                ...
                } else // Image-Shaders
                {
                    ...
                    fragmentShader = fragmentShader.replace("<???>", RenderSupport.getTextureLookupFlags(mTexture.format,
                                                                                                         mTexture.mipMapping,
                                                                                                         mTexture.repeat,
                                                                                                         smoothing));
                }
        ...
        private static function getImageProgramName(tinted:Boolean, mipMap:Boolean = true, repeat:Boolean = false, format:String = "bgra",
