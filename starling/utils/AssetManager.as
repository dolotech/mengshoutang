package starling.utils {
    ...
    public class AssetManager extends EventDispatcher {
        ...
        protected var mQueue:Array;
        private var mIsLoading:Boolean;
//		private var mTimeoutID:uint;

        protected var mTextures:Dictionary;
        protected var mAtlases:Dictionary;
        protected var mSounds:Dictionary;
        protected var mXmls:Dictionary;
        protected var mObjects:Dictionary;
        protected var mByteArrays:Dictionary;
        ...
        public function AssetManager(scaleFactor:Number = 1, useMipmaps:Boolean = false) {
            mVerbose = Capabilities.isDebugger;
        ...
        public function getTexture(name:String):Texture {
            if (name in mTextures)
                ...
                JTLogger.error("[AssetManager.getTexture] Can't Find The Texture : " + name);
                return null;
            }
        }
...
//			clearTimeout(mTimeoutID);
                ...
                if (rawAsset is Array) {
                    ...
                    if (!rawAsset["exists"]) {
                        ...
                        if (rawAsset["isDirectory"]) {
                            var fileArr:Array = _directoryDic[rawAsset["url"]];
                            if (!fileArr) {
                                fileArr = rawAsset["getDirectoryListing"]();
                                _directoryDic[rawAsset["url"]] = fileArr;
                            }
                            enqueue.apply(this, fileArr);
                        } else {
        ...
        private var _directoryDic:Object = {};
         ...
         */
        public function loadQueue(onProgress:Function, isFileLoader:Boolean = false):void {
...
//				throw new Error("The queue is already being processed");
            ...
            var xmlBytes:Vector.<ByteArray> = new <ByteArray>[];
...
//			resume();
            ...
            function loadByCount():void {
                var tmpCount:int = 0;
                var loadIndex:int = 0;
                for (var i:int = 0; i < load_count; i++) {
                    nextLoader();
                }

                function onComplete():void {
                    tmpCount++;
                    currentRatio = (numElements - tmpCount) ? 1.0 - ((numElements - tmpCount) / numElements) : 1.0;
//					Starling.juggler.delayCall(nextLoader,0.02);
                    nextLoader();
                    if (tmpCount == numElements) {
                        processXmls();
                        mQueue.length = 0;
                        mIsLoading = false;
                ...
                function nextLoader():void {
                    if (loadIndex == numElements)
                        return;
                    var file:Object = mQueue[loadIndex];
                    loadIndex++;
                    processRawAsset(file.name, file.asset, xmls, xmlBytes, progress, onComplete, isFileLoader);
                }
            }
...
//				clearTimeout(mTimeoutID);
                processRawAsset(assetInfo.name, assetInfo.asset, xmls, xmlBytes, progress, resume);
            }
...
//						throw new Error("XML contents not recognized: " + rootNode);
        ...
        private function processRawAsset(name:String, rawAsset:Object, xmls:Vector.<XML>, xmlBytes:Vector.<ByteArray>, onProgress:Function,
                                         onComplete:Function, isFileLoader:Boolean = false):void {
            var canceled:Boolean = false;

            addEventListener(Event.CANCEL, cancel);
            loadRawAsset(name, rawAsset, progress, process, isFileLoader);

            function process(asset:Object, extension:String, isGotoNext:Boolean = true):void {
                var texture:Texture;
                var bytes:ByteArray;
                ...
                //atf+xml+scml合并解析
                    ...
                    while (tmp_bytes.bytesAvailable) {
                        ...
                        if (tmp_extension == "atf")
                            process(tmp_filebytes, tmp_extension, false);
                        else
                            process(new XML(tmp_filebytes), tmp_extension, false);
                    }
                    tmp_bytes.clear();
                    ...
                    texture.root.onRestore = function():void {
                        mNumLostTextures++;
                        loadRawAsset(name, rawAsset, null, function(asset:Object, extension:String):void {
                            try {
                    ...
                    if (extension == "scmlx") {
                        ...
                        xmlBytes.push(bytes);
                        onComplete();
                    } else if (AtfData.isAtfData(bytes)) {
                        ...
                        texture.root.onRestore = function():void {
                            ...
                            loadRawAsset(name, rawAsset, null, function(asset:Object, extension:String):void {
                                ...
                                try {
                                    ...
                                    trace(this, "Texture restoration failed: " + e.message);
                                }
                                asset.clear();
                                mNumRestoredTextures++;
                        ...
                        process(new XML(bytes), extension);
                        bytes.clear();
                    } else {
...
//		private var urlLoader:URLLoader;
        private function loadRawAsset(name:String, rawAsset:Object, onProgress:Function, onComplete:Function, isFileLoader:Boolean = false):void {
...
//					return;
                    ...
                    //用file加载更快
//				onFileLoaderComplete();
            }

            /*	function onFileLoaderComplete():void
                {
            ...
            function onLoaderComplete(event:Object):void {
                ...
                onComplete(event.target.content, extension);
            }
        }
...
//				throw new ArgumentError("Cannot extract names for objects of type '" + name + "'");
        ...
        private function byteArrayStartsWith(bytes:ByteArray, char:String):Boolean {
            ...
            if (length >= 4 && (bytes[0] == 0x00 && bytes[1] == 0x00 && bytes[2] == 0xfe && bytes[3] == 0xff) || (bytes[0] == 0xff && bytes[1] == 0xfe && bytes[2] == 0x00 && bytes[3] == 0x00)) {
                start = 4; // UTF-32
        ...
        public function addAtfTexture(name:String, bytes:ByteArray):void {
            ...
            if (AtfData.isAtfData(bytes)) {
                texture = Texture.fromAtfData(bytes, mScaleFactor, mUseMipMaps, null);
                texture.root.onRestore = function():void {
                    mNumLostTextures++;
                };
