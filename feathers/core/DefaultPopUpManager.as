...
package feathers.core
{
	...
	public class DefaultPopUpManager implements IPopUpManager
	{
		 ...
		 * @private
		 */
		public var _popUps:Vector.<DisplayObject> = new <DisplayObject>[];
		...
		public function addPopUp(popUp:DisplayObject, isModal:Boolean = true, isCentered:Boolean = true, customOverlayFactory:Function = null):DisplayObject
		{
			...
			this._root.addChild(popUp);
			JTGlobalFunction.autoAdaptiveSize(popUp);
		...
		public function removePopUp(popUp:DisplayObject, dispose:Boolean = false):DisplayObject
		{
			...
			if(index < 0)
			{
				trace("Display object is not a pop-up.");
				return null;
			}
			...
			if (SceneMgr.instance.isScene(CityFace) 
				&& PopUpManager.isEmpty()  
				&& DialogMgr.isEmpty()
				&& JTChatControllerComponent.isShowCityIcon) //�ر����촰��
			{
		...
		{
			var l:int = this._popUps.length;
			if (!l)
			{
				return;
			}
			while(this._popUps.length)
			{
				var child:DisplayObject = this._popUps[0] as DisplayObject;
				child = removePopUp(child, true);
				child.dispose();
				child = null;
			}
		}
		...
		public function centerPopUp(popUp:DisplayObject):void
		{
			...
			//JTLogger.info(popUp.x, " PopUpY" + popUp.y);
			if (popUp is JTPanel)
			{
				JTPanel(popUp).showBackground();
			}
			else if (popUp is JTSprite)
			{
				JTSprite(popUp).showBackground();
			}
		}
