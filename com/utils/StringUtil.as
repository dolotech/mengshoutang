package com.utils
{
    ...
    public class StringUtil
    {
         ...
         *
         */
        public static function concatStr(concatStr:String, ... args):String
        {
            var len:int=args.length;
            var newStr:String=args[0];

            for (var i:int=1; i < len; i++)
            {
                newStr+=concatStr + args[i];
            }
            return newStr;
        }
         ...
         *
         */
        public static function replceTag(str:String, tag:String, ... args):String
        {
            for each (var key:String in args)
            {
                str=str.replace(tag, key);
            }
            return str;
        }
         ...
         *
         */
        public static function replaceNumber(value:String):String
        {
         ...
         *
         */
        public static function charCount(str:String):int
        {
         ...
         *
         */
        public static function getChar(str:String, needCount:int):String
        {
            var len:int=str.length;
            var count:Number=0;
        ...
        private static const PRICE:int=10000;

        public static function changePrice(price:int, isAddW:Boolean=true):String
        {
            var point:int=(price % PRICE / (PRICE / 10));
        ...
        public static function changePriceText(price:int, txt:TextField, endTxt:TextField, isAddW:Boolean=true):void
        {
            var firstNum:int=int(price.toString().substr(0, 1));
            txt.text=isAddW ? changePrice(price, false) : price.toString();

            if (endTxt == null)
                return;
            endTxt.text=price >= PRICE && isAddW ? "w" : "";
            endTxt.x=txt.x + txt.textBounds.width + (firstNum == 1 ? 6 : 2);
